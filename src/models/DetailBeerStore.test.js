...
jest.mock('../services/beersService', () => ({
    getBeers: () => {
...
it('creates DetailBeerStore instance', () => {
    const store = DetailBeerStore.create({
        state: 'initial',
    });
...
it('throws on invalid DetailBeerStore model', () => {
    expect(() => {
        DetailBeerStore.create({});
    }).toThrow();
});

it('fetches similar beer', async () => {
    const store = DetailBeerStore.create({
        state: 'initial',
    });

    await store.fetch(192);

    expect(store.data.id).toBe(192);
